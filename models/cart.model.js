...
exports.getcartByUserID = async (id) =>{
    try {  
      ...
      return new Promise((resolve, reject)=>{   
        ...
        db.query(`SELECT carts.*  , carts.id as cart_id  , products.*  FROM carts  join products on carts.product_id=products.id where carts.user_id = '${id}' or  carts.temp_user_id = '${id}'  order by carts.id desc `,  (error, elements)=>{
              if(error){
                  return reject(error);
              }
              return resolve(elements);
